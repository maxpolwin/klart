...
> **The app lives in [`NoschenMac/`](NoschenMac/).** See **[NoschenMac/README.md](NoschenMac/README.md)** for the full guide — highlights, build & distribution, provider setup, architecture, and the security/encryption model. This page is the short version.
...
- **Local-first & private** — notes live in `~/Library/Application Support/Noschen/Notes`; API keys live in the macOS Keychain; no telemetry. Optional at-rest note encryption with app lock, Touch ID unlock, and auto-lock.
...
```bash
git clone <repo-url>
cd Noschen/NoschenMac

swift run                  # run in development
swift test                 # run the unit tests
bash Scripts/make-app.sh   # build a distributable Noschen.app → dist/
```

CI builds, tests, and packages the app on every push — see [`.github/workflows/macos-app.yml`](.github/workflows/macos-app.yml). Full build, signing, and notarization instructions are in [NoschenMac/README.md](NoschenMac/README.md#build--run).

## Repository layout

```
NoschenMac/     The macOS app (SwiftPM: NoschenKit library + Noschen app)
...
- **[NoschenMac/README.md](NoschenMac/README.md)** — the complete guide (start here)
