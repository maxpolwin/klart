#!/usr/bin/env bash
# Notarizes and staples a signed build artifact — dist/Noschen.dmg (what you
# actually hand to users to install) or dist/Noschen.app. This is the step
...
#        (defaults to dist/Noschen.dmg if it exists, else dist/Noschen.app)
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ -z "$TARGET" ]; then
  if [ -e "dist/Noschen.dmg" ]; then
    TARGET="dist/Noschen.dmg"
  else
    TARGET="dist/Noschen.app"
  fi
fi
