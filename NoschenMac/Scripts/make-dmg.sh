#!/usr/bin/env bash
# Packages dist/Noschen.app (built by Scripts/make-app.sh) into a
# drag-to-install disk image: dist/Noschen.dmg.
#
...
#   bash Scripts/notarize-app.sh dist/Noschen.dmg
set -euo pipefail
cd "$(dirname "$0")/.."

APP="dist/Noschen.app"
DMG="dist/Noschen.dmg"
VOLUME_NAME="Noschen"
...
if [ "$SIGN_IDENTITY" = "-" ]; then
  ...
  echo "Next: bash Scripts/notarize-app.sh dist/Noschen.dmg"
fi
