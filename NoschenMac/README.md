...
- **Optional note encryption + app lock** — Settings → Security encrypts every note file at rest (ChaCha20-Poly1305; key derived from your password via PBKDF2-HMAC-SHA256, 600k rounds) and locks the app behind that password, with optional Touch ID unlock. Each ciphertext additionally authenticates the note's identity (AAD), so a sealed file copied under another note's name fails to decrypt instead of impersonating it. On Apple Silicon / T2 Macs the Touch ID key copy is encrypted to a **Secure Enclave** P-256 key whose private half never leaves the hardware and demands user presence on every decrypt; Macs without an enclave fall back to a user-presence Keychain control. Lock any time with `⌘L`; the app starts locked. There is no recovery backdoor: a forgotten password (with Touch ID off) means the notes are gone — keep a markdown export. FIDO2 hardware keys (YubiKey) are not supported: macOS exposes no public API to derive stable encryption secrets from a security key for app-local vaults, so password + Touch ID is the honest option.
